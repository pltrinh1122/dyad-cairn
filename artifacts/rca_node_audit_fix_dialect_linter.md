...
The Dialect Linter (`skills/dialect_linter.py`) serves as the CSI Guardrail to enforce UI Dialect invariants (e.g. mechanically invoking `bin/reflect` and presenting the CSS output). The linter failed to flag a violation when the UI presentation was physically omitted. 

## 2. Root Cause
1. **XML Parsing Failure**: The user's directive (`reflect:`) was encapsulated in the physical layer's `<USER_REQUEST>` tag. The linter evaluated `.startswith("reflect:")`, which failed.
