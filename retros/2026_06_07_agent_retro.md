...
- Activating `node_11_probe_todo_pipeline` to give your `todo:` ideas a formal, pre-DAG holding pen in the local substrate so we don't pollute the SPAOR loop.

## Stop
- Hallucinated transitions or generative execution. We must never update the Ledger or DAG manually; the new mechanical guardrails must be respected completely.
