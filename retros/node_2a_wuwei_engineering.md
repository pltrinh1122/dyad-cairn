...
2. **The Wu-Wei Intent Gate:**
   The initial Red/Green TDD workflow conceptually separated intent from execution, but contained a violent architectural contradiction: Instructing the Operator to "merge" the Red PR. Since the Red PR is designed to fail CI, GitHub physically blocks the merge. Forcing the Operator to override branch protection is the opposite of wu-wei. A Red PR is read-only; approval must be signaled locally (e.g., via chat), leaving the actual Merge action exclusively for the passing Green PR.
   
3. **The Lexicon Enforcement:**
