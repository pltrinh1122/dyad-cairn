...
> - **Form + spine conditions (C1–C8, C10): n=1** — the 2026-07-08 materialization arc
>   (`DYAD_LEDGER.md` REFLECT `2026-07-08 21:07:16`).
> - **The metaphor budget (C7): n=2** — materialization (same REFLECT, SH) **and** the
>   refinement arc (PR #154, commit `313a317`).
> - **Falsifier-fidelity + grounding conditions (C9, C11–C13): n=1** — the refinement
>   arc (audit `wf_8918028a-253`, PR #154).
...
**Exemplar:** `dyad-cairn/README.md` (this repo — the reference output).
...
- **LINT** — mechanically decidable; `skills/readme_lint.py` checks it. The linter holds
  no opinion on register or meaning (Builder vs Enforcer): it may not judge taste.
...
- **OPERATOR** — a generative judgment (register, metaphor, emphasis); only a human can
  settle it.
...
metaphor, which is bound by C5.

**C3 — The status never overstates.** `belief.status` is `hypothesis` or `theory` —
never `conjecture` (imports a proof that is not coming), never `settled`; `grade` never
reads `settled`. *Violation:* a settled/conjecture status, a "settled" grade. *Verifier:*
LINT (status/dogma locks); the `grade`/`coverage` *values* are AUDIT (C4), not LINT.
...
evidence, re-derived from its ledger — the fixed-value fields (C1) are the only ones
copied. `grade` caps at `survives` (an unexercised README says so, e.g. `"unexercised —
no arc survived yet"`); `coverage` uses the E-scale (`E0` = no outside attack; `En` per
independent outside attack survived — raising confidence, not status). *Violation:* an
exemplar's evidence string copied onto an unearned base (e.g. `survives — intra-dyad` on
a README no arc has survived). *Verifier:* AUDIT.

**C5 — Every metaphor is the dyad's own.** No metaphor — in body prose *or* in a
frontmatter comment — is carried untranslated from an exemplar. *Violation:* an
exemplar's figure appearing verbatim (bond's "hands you the knife", "pipeline", "engine").
*Verifier:* OPERATOR. *Origin (n=1):* materialization arc recorded the carry-anti-pattern
verbatim (`DYAD_LEDGER.md` REFLECT `2026-07-08 21:07:16`, Stop).

### Claims and spine

**C6 — Every claim hands over a knife.** The body contains numbered claims, and every
...
**C7 — One metaphor domain, two at most.** The document rests on a single physical
...
**C8 — The spine is fixed before the form.** Before any large prose draft exists, four
...
**C9 — Falsifier fidelity.** Each claim's refutation-extension matches its
...
**C10 — Proof of origin resolves, at whichever layer.** In the formal register, each
...
**C11 — The substrate under a claim exists.** Every artifact a claim presupposes (a
wrapper, a guard, a playbook) is present on disk. *Violation:* a claim standing on an
absent artifact. *Verifier:* AUDIT.

**C12 — Honesty-field values are consensus-set and register-stable.** `status`/`grade`/
...
**C13 — Gaps are surfaced, never smoothed.** A substrate gap found while grounding is
...
**C14 — The mechanical gate is green.** `python3 skills/readme_lint.py README.md` exits
0. *Violation:* nonzero exit. *Verifier:* LINT (self).

**C15 — It landed through the FSM, not around it.** The README reached `main` via
...
**C16 — The tool was vendored whole.** An adopting dyad holds `skills/readme_lint.py`
...
C4, C9, C10, C11, C12, C13 are decided here. Register passes silently drift claim
...
2. **The formal baseline** — the last committed formal version
   ...
   yet):** the Operator-approved spine (C8) serves as the falsifier-extension baseline,
   since it fixed one falsifier per claim before any register pass. *(This branch: n=0 —
   no dyad has run it.)*
3. **The physical substrate** — the artifacts each claim presupposes (C11).

The audit is adversarial by construction: a finding survives only if an independent pass
fails to refute it. This is what makes C9–C13 conditions rather than hopes.

## Invariants restated as the load-bearing conditions

If a dyad keeps only a few, keep these — they are the ones whose violation was lived, not
imagined: **C5/C7** (metaphors are yours, budget one–two), **C8** (spine before form),
**C9** (falsifier fidelity), **C10** (proof of origin resolves), **C12** (honesty fields
never weaken), **C13** (gaps surfaced, never smoothed). The rest are mechanically held by
`readme_lint.py` (C1–C3, C6, C10-lint, C14) or by the dyad's existing gates (C15–C16).
