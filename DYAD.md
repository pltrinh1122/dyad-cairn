...
## Craft (Dimension 1 & 6)
- **craft** — *Mastering the practice of turning shared challenges into reusable skills.* (The object is building the efficient, repeatable process itself; the playbooks are just the byproduct.)
- **craft_telos** — *Minimize the back-and-forth it takes to turn a messy challenge into a proven skill.* (Unit: discovery turns + structuring turns.)
- **craft_value** — **proof of origin.** (Every lesson must trace back to a real team's actual struggle.)
- **craft_invariant** — **Never fill gaps with guesses.** If finishing requires making up a missing piece because the real data has a gap, take the extra turn to find the truth. *(Derivation and tension reasoning offloaded to `reflect/2026-07-08-craft-refresh.md`).*
...
## 6 & 7. Ontology & Vocabulary
Our vocabulary reflects the physical extraction of stones:
...
- **Deferred Offload:** The Agent halts and defers to the Operator (`clip:` / `pin:` / `reflect:` / `d-reflect` / `d-land`) when resolving friction, navigating ambiguous signal, or making strategic steering choices. The dialect commands are executed exclusively via their deterministic scripts (`./bin/clip`, `./bin/pin`, `./bin/reflect`, `./bin/read`, `./bin/bind`, `./bin/d-reflect`, `./bin/d-land`) to physically enforce the append-only Ledger invariant.
...
- **Outbound Syncing:** Structural syncs (like Reflect summaries) must be routed autonomously to peer dyads (e.g., `dm/dyad-touchstone/`) without halting for PR gating.
- **`d-reflect` (adopted from dyad-bond, `github.com/pltrinh1122/dyad-bond`):** One token doing two jobs — fires the CSS reflect (Continue/Start/Stop, `kb/templates/reflect.md`) *and* updates `dyad-state/carry-forward.md`, the single-home resume ledger `SESSION_START` reads. Adopted because our own `reflect:` and any future resume-prep step were always going to be paired in the same turn — one job, not two, the same collapse bond made of its `reflect`/`stand-down` tokens. Only the base CSS form is settled upstream (survived 4 intra-dyad applications, kb-with-caveat); its Operator-provenance layer is not — re-grounded 2026-07-04 against bond's own re-derivation, which replaced an earlier per-line `(OR)` CONTINUE-tag with a standalone **SH (Should Have / Should Hold)** entry-type, verbatim-quote-grounded, still `CANDIDATE` with zero survived instances of the new grammar upstream. Ported as optional guidance in `kb/templates/reflect.md`, not asserted as settled here either — never smooth the mortar on an adopted discipline's own open gaps.
