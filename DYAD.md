...
- **Deferred Offload:** The Agent halts and defers to the Operator (`clip:` / `pin:` / `retro:` / `d-reflect`) when resolving friction, navigating ambiguous signal, or making strategic steering choices. The dialect commands are executed exclusively via their deterministic scripts (`./bin/clip`, `./bin/pin`, `./bin/retro`, `./bin/read`, `./bin/bind`, `./bin/d-reflect`) to physically enforce the append-only Ledger invariant.
...
- **`d-reflect` (adopted from dyad-bond, `github.com/pltrinh1122/dyad-bond`):** One token doing two jobs — fires the CSS+OR retro (Continue/Start/Stop, provenance-tagged per `kb/templates/retro.md`) *and* updates `dyad-state/carry-forward.md`, the single-home resume ledger `SESSION_START` reads. Adopted because our own `retro:` and any future resume-prep step were always going to be paired in the same turn — one job, not two, the same collapse bond made of its `reflect`/`stand-down` tokens. The OR (Operator-Reflect) provenance tag ports the *credit* direction only (settled, survived 4 applications upstream); the *debit* direction (flagging Operator drift) stays `CANDIDATE` upstream and is not asserted as settled here either — never smooth the mortar on an adopted discipline's own open gaps.
