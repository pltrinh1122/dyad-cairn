...
def test_fsm_process_retro_success():
    with patch('subprocess.run') as mock_sub_run:
        ...
        mock_sub_run.assert_any_call("python3 skills/reflect_linter.py \"fake_path.md\"", shell=True, capture_output=False, text=False)
        
        # Assert Ledger Manager is called
        mock_sub_run.assert_any_call("python3 skills/ledger_manager.py reflect \"test summary\" \"fake_path.md\"", shell=True, capture_output=False, text=False)

def test_fsm_process_reflect_linter_failure():
    with patch('subprocess.run') as mock_sub_run, patch('sys.exit') as mock_exit, patch('builtins.print') as mock_print:
        ...
        mock_print.assert_any_call("🚨 CSI GUARDRAIL BLOCK: Reflect does not match CSS template.")
        mock_exit.assert_called_once_with(1)

def test_bin_retro_delegates_to_fsm():
    with open("bin/reflect", "r") as f:
        content = f.read()
        assert "python3 skills/flow_state_manager.py reflect" in content, "bin/reflect must delegate to flow_state_manager.py"
