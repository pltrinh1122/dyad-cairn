...
def test_dialect_linter_with_mock_transcript(tmp_path):
    ...
    assert result.returncode == 0, f"Failed: {result.stdout}"
        # Scenario 2: User says `reflect:` but Agent fails to invoke `bin/reflect`
    with open(transcript_path, "a") as f:
        f.write(json.dumps({"type": "USER_INPUT", "content": "reflect:\n", "step_index": 5}) + "\n")
        f.write(json.dumps({"type": "PLANNER_RESPONSE", "tool_calls": [{"args": "something else"}]}) + "\n")
        
    result = subprocess.run(["python3", str(mock_script)], env=env, capture_output=True, text=True)
    assert result.returncode == 1
    assert "CSI GUARDRAIL BLOCK" in result.stdout
    assert "User issued 'reflect:' but Agent failed to mechanically invoke 'bin/reflect'" in result.stdout
    ...
    # Scenario 4: User says `reflect:`, Agent runs bin/reflect but fails to print CSS template
    with open(transcript_path, "w") as f:
        f.write(json.dumps({"type": "USER_INPUT", "content": "reflect: test\n", "step_index": 6}) + "\n")
        f.write(json.dumps({"type": "PLANNER_RESPONSE", "content": "Locked to ledger.", "tool_calls": [{"args": '{"CommandLine": "./bin/reflect test test.md"}'}]}) + "\n")
        
    result = subprocess.run(["python3", str(mock_script)], env=env, capture_output=True, text=True)
    assert result.returncode == 1
    assert "failed to explicitly present the CSS template in the chat UI" in result.stdout
    
    # Scenario 5: User says `reflect:`, Agent runs bin/reflect and prints CSS template
    with open(transcript_path, "w") as f:
        f.write(json.dumps({"type": "USER_INPUT", "content": "reflect: test\n", "step_index": 7}) + "\n")
        f.write(json.dumps({"type": "PLANNER_RESPONSE", "content": "📋 [MECHANICAL UI PRESENTATION: REFLECT SUMMARY]\nIt worked.", "tool_calls": [{"args": '{"CommandLine": "./bin/reflect test test.md"}'}]}) + "\n")
...
def test_prevent_auto_rub_todo(tmp_path):
    env = os.environ.copy()
    conv_id = "test_conv_id_todo_autorub"
    env["ANTIGRAVITY_CONVERSATION_ID"] = conv_id
    ...
    # Valid: no ask_question
    with open(transcript_path, "w") as f:
        f.write(json.dumps({"type": "USER_INPUT", "content": "todo: something"}) + "\n")
        f.write(json.dumps({"type": "PLANNER_RESPONSE", "tool_calls": []}) + "\n")
    ...
    # Invalid: ask_question invoked
    with open(transcript_path, "w") as f:
        f.write(json.dumps({"type": "USER_INPUT", "content": "todo: something else"}) + "\n")
        f.write(json.dumps({"type": "PLANNER_RESPONSE", "tool_calls": [
            {"function": {"name": "default_api:ask_question", "arguments": "{\"questions\": [{\"question\": \"WHY?\"}]}"}}
        ]}) + "\n")
    ...
    result = subprocess.run(["python3", str(mock_script)], env=env, capture_output=True, text=True)
    assert result.returncode == 1
    assert "Auto-Rub" in result.stdout
