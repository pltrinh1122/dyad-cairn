...
def process_d_land(carry_forward_note):
    ...
    print("[FLOW] Asserting d-land Pull Request invariant...")
    try:
        branch = run_cmd("bin/git rev-parse --abbrev-ref HEAD")
        if branch != "main":
            if os.environ.get("DYAD_TEST_ENV") == "1":
                print("[FLOW] Test environment detected; skipping push and PR generation.")
            else:
                print(f"[FLOW] Pushing branch {branch} to remote...")
                run_cmd(f"bin/git push -u origin {branch}")
                
                print("[FLOW] Generating Pull Request for d-land...")
                sys.path.append('.')
                from skills.github_client import create_pr
                title = f"[{branch}] Arc Landing"
                body = f"Automated PR generated by d-land discipline.\n\n{final_note}"
                try:
                    pr_url = create_pr(title, body)
                    print(f"[FLOW] PR successfully opened at: {pr_url}")
                except Exception as e:
                    print(f"[FLOW] Warning: PR might already exist or failed to create: {e}")
        else:
            print("[FLOW] Currently on main branch; skipping PR generation.")
    except Exception as e:
        print(f"[FLOW] Warning: failed to generate d-land PR: {e}")
